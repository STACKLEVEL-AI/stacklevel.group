...
export default function Header() {
  ...
  // With localePrefix: "always", the pathname is like /en/services or /ru/services
  // To switch language, we replace /en with /ru or vice versa
  // Handle root path case (/, /en, /ru) by adding the other locale
  const switchToEnHref = pathname === '/ru' || pathname.startsWith('/ru/') 
    ? pathname.replace(/^\/ru/, '/en') 
    : pathname === '/en' || pathname.startsWith('/en/')
      ? pathname 
      : '/en' + (pathname === '/' ? '' : pathname);
      
  const switchToRuHref = pathname === '/en' || pathname.startsWith('/en/')
    ? pathname.replace(/^\/en/, '/ru')
    : pathname === '/ru' || pathname.startsWith('/ru/')
      ? pathname
      : '/ru' + (pathname === '/' ? '' : pathname);

  const localeLinkClass = (isActive: boolean) =>
    `text-xs font-bold uppercase tracking-wide ${isActive ? "text-[var(--accent)] cursor-default" : "text-[var(--black)] hover:text-[var(--accent)]"}`;
      ...
      <div className="width-wrapper relative flex min-h-[84px] items-center justify-between py-3">
        ...
        <div className="hidden items-center gap-3 xl:flex">
          <div className="flex items-center gap-2">
            {isEnglish ? (
              <span className={localeLinkClass(true)}>EN</span>
            ) : (
              <NextLink href={switchToEnHref} className={localeLinkClass(false)}>
                EN
              </NextLink>
            )}
            {isRussian ? (
              <span className={localeLinkClass(true)}>RU</span>
            ) : (
              <NextLink href={switchToRuHref} className={localeLinkClass(false)}>
                RU
              </NextLink>
            )}
          </div>
