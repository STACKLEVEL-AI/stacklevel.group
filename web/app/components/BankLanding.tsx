...
const roles: Role[] = [
  {
    ...
    hint: { ru: "Digital-стратегия конкурентов", en: "Competitor digital strategy" },
    agents: [5, 4, 3],
  },
  {
    id: "product",
    label: { ru: "Продуктовый менеджер", en: "Product manager" },
    hint: { ru: "Ставки, ниши, пакет к комитету", en: "Rates, whitespace, committee pack" },
    agents: [1, 2, 4, 6, 3],
  },
  {
    id: "director",
    label: { ru: "Директор направления", en: "Business-line director" },
    hint: { ru: "Дайджест, сигналы, позиция", en: "Digest, signals, strategic position" },
    agents: [8, 1, 2, 3, 10],
  },
  {
    id: "board",
  ...
  return (
    <div className={styles.page}>
      <section className={`${styles.hero} ${styles.dotBg}`}>
        <div className={styles.container}>
          <div className={styles.heroEyebrow}>
            {locale === "ru" ? "#Продуктовая аналитика" : "#Product analytics"}
          </div>
          <h1 className={styles.heroTitle}>
            {locale === "ru" ? (
              <>
                БАНК ПРИНИМАЕТ <span className={styles.accent}>ПРОДУКТОВЫЕ РЕШЕНИЯ</span> БЫСТРЕЕ РЫНКА
              </>
            ) : (
              <>
                A BANK MAKES <span className={styles.accent}>PRODUCT DECISIONS</span> FASTER THAN THE MARKET
              </>
            )}
          </h1>
      ...
      <section className={styles.section}>
        <div className={styles.container}>
          <div className={styles.sectionLabel}>{sectionLabels.change[locale]}</div>
          <h2 className={styles.sectionTitle}>
            {locale === "ru" ? (
              <>
                ОТ РУЧНОГО ОБЗОРА <span className={styles.accent}>К НЕПРЕРЫВНОЙ АНАЛИТИКЕ</span>
              </>
            ) : (
              <>
                FROM MANUAL REVIEW TO <span className={styles.accent}>CONTINUOUS ANALYTICS</span>
              </>
            )}
          </h2>
          ...
          <div className={styles.compareWrap}>
            <div className={styles.compareGrid}>
              <div className={`${styles.compareColumn} ${styles.compareBefore}`}>
                <div className={styles.compareLabel}>{locale === "ru" ? "Сейчас" : "Today"}</div>
                {beforeItems.map((item) => (
                  <div key={item.ru} className={styles.compareItem}>
                    <span className={styles.compareMarker}>&gt;</span>
                    <span>{item[locale]}</span>
                  </div>
                ))}
              </div>

              <div className={`${styles.compareColumn} ${styles.compareAfter}`}>
                <div className={styles.compareLabel}>{locale === "ru" ? "С AI-агентами" : "With AI agents"}</div>
                {afterItems.map((item) => (
                  <div key={item.ru} className={styles.compareItem}>
                    <span className={styles.compareMarker}>&gt;</span>
                    <span>{item[locale]}</span>
      ...
      <section className={`${styles.section} ${styles.sectionMuted}`}>
        <div className={styles.container}>
          <div className={styles.sectionLabel}>{sectionLabels.cases[locale]}</div>
          <h2 className={styles.sectionTitle}>
            {locale === "ru" ? (
              <>
                КАК ЭТО РАБОТАЕТ <span className={styles.accent}>НА ПРАКТИКЕ</span>
              </>
            ) : (
              <>
                HOW IT WORKS <span className={styles.accent}>IN PRACTICE</span>
              </>
            )}
          </h2>
              ...
              return (
                <Fragment key={scenario.id}>
                  <button
                    ...
                    onClick={() => setOpenScenarioId(isOpen ? null : scenario.id)}
                    aria-expanded={isOpen}
                  >
                    <span className={styles.cardNumber}>{String(scenario.id).padStart(2, "0")}</span>
                    <span className={styles.cardTitle}>{scenario.title[locale]}</span>
                    <span className={styles.cardResult}>{scenario.result[locale]}</span>
                  </button>

                  {isOpen ? (
                    <div className={styles.expandedCard}>
                      <p className={styles.expandedStory}>{scenario.story[locale]}</p>
                      <div className={styles.expandedOutcome}>{scenario.outcome[locale]}</div>
                      <div className={styles.expandedMeta}>
                        {locale === "ru" ? "Работает агент:" : "Powered by agent:"}{" "}
                        <span>{agents.find((agent) => agent.id === scenario.agentId)?.name[locale]}</span>
                      </div>
                    </div>
                  ) : null}
                </Fragment>
              ...
              <div key={stat.value}>
                <div className={styles.statValue}>
                  ...
                  <span className={styles.statAccent}>
                    {typeof stat.accent === "string" ? stat.accent : stat.accent[locale]}
                  </span>
                  {"suffix" in stat && stat.suffix ? <span className={styles.statSuffix}>{stat.suffix}</span> : null}
                </div>
      ...
      <section className={styles.section}>
        <div className={styles.container}>
          <div className={styles.sectionLabel}>{sectionLabels.roles[locale]}</div>
          <h2 className={styles.sectionTitle}>
            {locale === "ru" ? (
              <>
                КАЖДАЯ РОЛЬ <span className={styles.accent}>ПОЛУЧАЕТ СВОЕ</span>
              </>
            ) : (
              <>
                EVERY ROLE <span className={styles.accent}>GETS ITS OWN VIEW</span>
              </>
            )}
          </h2>
            ...
            {roles.map((role) => {
              ...
              return (
                <button
                  ...
                  onClick={() => setActiveRoleId(isActive ? null : role.id)}
                  aria-expanded={isActive}
                >
                  <span className={styles.roleName}>{role.label[locale]}</span>
                  <span className={styles.roleHint}>{role.hint[locale]}</span>
              ...
              <div className={styles.roleExpanded}>
                ...
                <div className={styles.rolePills}>
                  {activeRole.agents.map((agentId) => {
                    ...
                    return agent ? (
                      <span key={agent.id} className={styles.rolePill}>
                        {agent.name[locale]}
                      </span>
                    ) : null;
                  })}
      ...
      <section className={`${styles.section} ${styles.sectionMuted}`}>
        <div className={styles.container}>
          <div className={styles.sectionLabel}>{sectionLabels.agents[locale]}</div>
          <h2 className={styles.sectionTitle}>
            {locale === "ru" ? (
              <>
                10 АГЕНТОВ. <span className={styles.accent}>ОТ ДАННЫХ ДО РЕШЕНИЙ.</span>
              </>
            ) : (
              <>
                10 AGENTS. <span className={styles.accent}>FROM DATA TO DECISION.</span>
              </>
            )}
          </h2>

          {(["specialist", "orchestrator", "executive"] as const).map((layer) => (
            <div key={layer} className={styles.agentLayer}>
              <div className={styles.layerLabel}>{layerLabels[layer][locale]}</div>
              <div className={styles.agentsGrid}>
                {agents
                  .filter((agent) => agent.layer === layer)
                  .map((agent) => (
                    <article key={agent.id} className={styles.agentCard}>
                      <div className={styles.agentNumber}>{String(agent.id).padStart(2, "0")}</div>
                      <h3 className={styles.agentName}>{agent.name[locale]}</h3>
                      <p className={styles.agentDesc}>{agent.desc[locale]}</p>
                      <div className={styles.agentValue}>{agent.value[locale]}</div>
                    </article>
                  ))}
              </div>
            </div>
        ...
        <div className={styles.container}>
          <div className={styles.demoGrid}>
            <article className="stack-panel-dark p-5 md:p-6">
              <div className={styles.demoLabel}>{locale === "ru" ? "Пилотный запуск" : "Pilot launch"}</div>
            ...
            <article className="stack-panel bg-white p-5 md:p-6">
              <div className={styles.demoCardLabel}>
                {locale === "ru" ? "Пилот на данных MyFin" : "Pilot on MyFin data"}
              </div>
              <div className={styles.demoCardTitle}>
                {locale === "ru" ? "2 недели до первой карты рынка" : "2 weeks to your first market map"}
              </div>
              ...
              <div className={styles.demoFacts}>
                <span className={styles.demoFact}>{locale === "ru" ? "21 банк" : "21 banks"}</span>
                <span className={styles.demoFact}>{locale === "ru" ? "8 доменов" : "8 domains"}</span>
                <span className={styles.demoFact}>{locale === "ru" ? "2 недели" : "2 weeks"}</span>
              </div>
            </article>
          </div>

          <div className={styles.demoFormWrap}>
            <Suspense fallback={<div className="stack-panel bg-white p-6 md:p-8">{locale === "ru" ? "Загрузка формы..." : "Loading form..."}</div>}>
